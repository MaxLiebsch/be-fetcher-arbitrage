export const packs = [
  {
    input: "Holle - Bio-Fol­ge­milch 2 aus Zie­gen­milch - 0,4 kg - 6er Pack",
    package: 6,
    type: "pack",
  },
  {
    input: "Hama SpotLight Steady 120 Set",
    package: null,
  },
  {
    input: "Compeed Blasenpflaster Mixpack - Hydrokolloid-Pflaster in verschiedenen Gren - gegen Blasen an Fersen, Zehen und fr kleine Blasen, 6 stck",
    package: 6,
  },
  {
    input: "Gillette Fusion 5 Rasierklingen für Rasierer, 14 Ersatzklingen für Nassrasierer Herren mit 5-fach Klinge, Made in Germany",
    package: 14,
  },

  { input: 'Kneipp Brennnessel Dragees, natürlich durchspülend, auch bei Entwässerungskuren, 1 x 90 Dragees | 90 Stück (1er Pack)',
    package: 90,
  },
  {
    input: 'Sheba Katzensnacks Creamy Snacks, 18 praktische Katzenleckerli Sticks, 18x12g (1 Packung) – in den Varietäten Huhn, Lachs (MSC zertifiziert) und Rind zum aus der Hand schlecken',
    package: 18,
  },
  {
    input: "Pampers Pampers Ninjamas Nachthöschen / Höschenwindeln für Mädchen (27-43kg), 54 Pyjama Höschen, 8-12 Jahre, MONATSBOX, absorbierende Windelhöschen, Auslaufschutz für die ganze Nacht",
    package: 54,
  },
  {
    input: 'TRIXIE glutenfreie Hhnchen Kaurolle Denta Fun Chicken Chewing Rolls, 12 cm, 30 St./240 g - 31378 (Packung mit 2)',
    package: 2,
  },
  {
    input: "Zeller Weinregal Fichte für 25 Flaschen (13172)",
    package: null,
  },
  {
    input: "Zeller Weinregal Fichte 25 Flaschen (13172)",
    package: 25,
  },
  {
    input: "12 Pck. 3M Deutschland Atemschutzmaske 8710 (VE20) Atemschutzmasken 7000034734",
    package: 12,
  },
  {
    input: "Bosch 1000x Klammer Typ 53 (Textilien/Gewebe, Teppich, Akustikplatten, Rasenteppich, 11.4 x 0.74 x 8 mm, Zubehr Tacker)",
    package: 1000,
  },
  {
    input:
      "Lavazza Crema e Gusto Classico, 108 Kaffeepads für Espressomaschinen",
    package: 108,
  },
  {
    input: "WELLER WR 2002 - Lötstation, Entlötstation, WR 2002 Set, 300 W, 2-Kanal, ESD",
    package: null
  },
  {
    input:
      "Radeon RX 7800 XT GAMING OC 16G, Grafikkarte RDNA 3, GDDR6, 2x DisplayPort, 2x HDMI 2.1",
    package: null,
  },
  {
    input: "Q4AC - Ladegerät für Akkupacks Q4AC, 1-4s, max. 5 A, 4x 50 W",
    package: null,
  },
  {
    input:
      "Radeon RX 7800 XT GAMING OC 16G, Grafikkarte RDNA 3, GDDR6, 2x Displayport, 2x HDMI 2.1",
    package: null,
  },
  {
    input: "Duss PK 600 SET",
    package: null,
  },
  {
    input: "Paulmann LED Base rund 10 x max. 10W GU10 weiß (934.00)",
    package: null,
  },
  {
    input:
      "APIFONDA Futterteig 12 x 1 KG Bienenfutter Südzucker Imkerei Futter für Bienen",
    package: 12,
  },
  {
    input: "Ferrero Kinder Bueno - 30 Riegel - Schokolade Schokoriegel",
    package: 30,
  },
  {
    input:
      "o.b. Original Tampons normal mit geschwungenen Rillen Saugfähig 64er",
    package: 64,
  },
  {
    input: "JBL Bar 800",
    package: null,
  },
  {
    input: "AMD Ryzen 7 5800X Boxed",
    package: null,
  },
  { input: "Adult Huhn mit Lachs 2 kg 3 Packungen", package: 3 },
  { input: "24 Flaschen San Pellegrino SANBITTER a´ 98ml", package: 24 },
  {
    input:
      "Gourmet Perle Erlesene Streifen Katzenfutter nass, mit Lachs, 26er Pack (26 x 85g)",
    package: 26,
  },
  {
    input: "Max Factor Pastell Compact Powder 10 Pastell, 2er Pack (1 x 20 ml)",
    package: 2,
  },
  {
    input: "Polaroid Color Film für 600 - Doppelpack",
    package: 2,
  },
  {
    input: "Pampers Harmonie Aqua Baby Feuchttücher Box, 720 Tücher (15 x 48), sanfter Hautschutz für zarte Haut mit 99% Wasser",
    package: 15,
  },
  {
    input:
      "L'Oréal Paris Permanente Haarfarbe, Haarfärbeset mit Coloration und Farbglanz-Pflegebalsam, Préférence, P11 Kühles Intensives Schwarz (Manhattan), 3er Set",
    package: null,
  },
  {
    input:
      "12 Dosen Airwaves Menthol & Eukalyptus o. Zucker a 50 Dragees Kaugummi",
    package: 12,
  },
  {
    input: "STABILO Point 88 50 Stück Metallgehäuse",
    package: 50,
  },
  {
    input: "Eschenbach powerlux 7x LED (8000K)",
    package: null,
  },
  {
    input:
      "4x new Retro Wheels RONAL R10 Turbo 7x15 4x100 ET:37 VW Golf 2 3 Jetta - USA",
    package: null,
  },
  {
    input: 'Pampers Windeln Größe 7 (15kg+) Baby-Dry, Extra Large, MONATSBOX, bis zu 12 Stunden Rundum-Auslaufschutz, (1er Pack) 132 Stück',
    package: 132,
  },
  {
    input:
      "Philips Hue Smart Weiß E27 LED Leuchtmittel Viererpack Dimmbar 800lm Bluetooth",
    package: 4,
  },
  {
    input:
      "4 x Unterstellbock Stützbock je 900kg für Auto Wohnwagen Wohnmobil von CARTREND",
    package: 4,
  },
  {
    input: "EFB-Elektronik Spleißschutz TS0338/96 (VE150) Spleißschutz 53100.32-150",
    package: 150,
  },
  {
    input: "Cartrend Caravan Stützbock 4er-Set (10306)",
    package: null,
  },
  {
    input: "Kessel medintim Oral Safe Latexschutztuch Vanille (8 Stk.)",
    package: 8,
  },
  {
    input: "ORAL SAFE Latexschutztuch Vanille 8 St",
    package: 8,
  },
  {
    input:
      "Gourmet PURINA GOURMET Mon Petit Intense Katzenfutter nass, Geflügel-Variationen, 8er Pack (8 x 6 Beutel à 50g) Paket kann variieren",
    package: 8,
  },
  {
    input: "LGMDM 4B6602458 - Fahrzeugantenne 4x4 MiMo 4G/5G + 2x2 MiMo WIFI + GPS/GNSS - Schwar",
    package: null,
  },
  {
    input: "LGMDM 4B6602458 - Fahrzeugantenne 4x4 MIMO 4G/5G + 2x2 MiMo WIFI + GPS/GNSS - Schwar",
    package: null,
  },
  {
    input: "BCS600 - Fernglas, 12x32, Prisma K9, schwarz",
    package: null,
  },
  {
    input:
      "5x Rummo Pasta Integrale Fusilli N.48 Vollkornnudeln 500g+Polpa 400g",
    package: 5,
  },
  {
    input: "Rohrmotor PLUS Mini 6 Nm - Rohrmotor Plus Mini, 6 Nm für 40er Welle",
    package: null,
  },
  {
    input:
      "Philips Hue White & Color Ambiance Centura Deckenspot Einbauspot sw • 3er Pack",
    package: 3,
  },
  {
    input: "4er Satz Sparco Radzierblenden Lazio 15-Zoll Schwarz/Grau",
    package: 4,
  },
  {
    input: "3x Brunox epoxy Roststopp & Grundierung 400ml Sprühdose",
    package: 3,
  },
  {
    input: "Oral-B iO Ultimative Reinigung Aufsteckbürsten schwarz (4 Stk.)",
    package: 4,
  },
  {
    input:
      "JACOBS Lungo 8 Intenso 200 Nespresso®* kompatible Kaffeekapseln +2x 20 gratis",
    package: null,
  },
  {
    input: "Cenovis Klare Gemüsebrühe 4x12 Stück",
    package: 4,
  },

  {
    input: "Fast & Furious - 10-Movie-Collection [10 DVDs]",
    package: null,
  },
  {
    input: "Wahl Edelstahl-Aufschiebekamm Set 1247-7440",
    package: null,
  },
  {
    input: "Lowa Care Set (831101)",
    package: null,
  },
  {
    input: "Metabo KGS 254 M Set (613254900)",
    package: null,
  },
  {
    input: "Rothenberger Pressbackenset Standard SV15-18-22-28 (015060X)",
    package: null,
  },
  {
    input:
      "Bosch Professional 18V System Akku GBA 18V 4.0Ah (2 x 65 Waschladungen) hochkonzentriert",
    package: 2,
  },
  {
    input:
      "GP 10023AC4 Hoog Voltage Alkaline Knopfzelle 23A (MS21 / MN21), 4er Blister",
    package: 4,
  },
  {
    input: "Sheba Multipack Soup mit Huhn | 10 Packungen 4 x 40g",
    package: 10,
  },
  {
    input:
      "Professional Akku-Rasenmäher RASARRO 36/40, 36Volt (2x18V) rot/schwarz, 2x Li-Ionen Akku 4,0Ah",
    package: null,
  },
  {
    input: "PILOT FriXion Ball 07 / 0,35 mm - Set mit 15 Farben",
    package: null,
  },
  {
    input:
      "Akku-Rasenmäher DLM539PT2, 36Volt (2x18Volt) blau/schwarz, 2x Li-Ionen Akku 5,0Ah, mit Radantrieb",
    package: null,
  },
  {
    input:
      "Bachmann CONI Modulträger 3-fach, 2x Strom, 2x USB-A, Steckdosenleiste",
    package: null,
  },
  {
    input: "DeLOCK Steckdosenleiste 4-fach, 3x USB",
    package: null,
  },
  {
    input: "Bachmann CONI Modulträger 3-fach, 3x Strom, Steckdosenleiste",
    package: null,
  },
  {
    input: "Carrera GO!!! Build 'n Race - Racing Set 6.2, Rennbahn",
    package: null,
  },
  {
    input: "RM650, PC-Netzteil schwarz, 4x PCIe, Kabel-Management, 650 Watt",
    package: null,
  },
  {
    input: "Shelly Plus Plug S 4er Sparpack, Schaltsteckdose, schwarz",
    package: 4,
  },
  {
    input: "Sheba Nassfutter Katze Soup mit Huhn, Multipack (4x40 g), 160 g",
    package: 4,
  },
  {
    input: "FABER-CASTELL Spitzdose Grip 2001 3-fach silber",
    package: null,
  },
  {
    input: "Pelikan Zeichenblock A3 sortiert 10 Blatt 100g/qm",
    package: null,
  },
  {
    input:
      "Müller - Toy Place - City Team Set - 5 Fahrzeuge, 3-fach sortiert, 1 Stück Spiel",
    package: 1,
  },
  {
    input: "folia Faltblätter aus Origamipapier 96 Blatt 19 x 19 cm",
    package: null,
  },
  {
    input: "beaphar Hundesnack Zahnpflege Sticks 7 St./112g",
    package: 7,
  },
  {
    input:
      "(1L|27,78) 6x150ml Fa Purple Passion 48H Deospray 0% Aluminium Veilchenblüten",
    package: 6,
  },
  {
    input: "folia Stickerrolle UNICORN 352 Teile",
    package: null,
  },
  {
    input: "KUM Holzanspitzer Wood 2",
    package: null,
  },
  {
    input: "Bench. Hüfttasche dunkelgrau 1700",
    package: null,
  },
  {
    input: "BIG 800001191 - Big Bobby Car - Pylonen Set 4 - Neu",
    package: 4,
  },
  {
    input:
      "1 x 4er Pack Cenovis Klare Gemüsebrühe nach Schweizer Rezept rein pflanzlich",
    package: 4,
  },
  {
    input:
      "Neues AngebotMülltonnenbox Gerätebox Westmann 7x3 Geräteschuppen Gartenbox 3er Verkleidung",
    package: 3,
  },
  {
    input: "Scheppach HC06 Set",
    package: null,
  },
  {
    input: "Maybach Grauer Burgunder (Pinot Grigio) 12% vol 6 x 75cl",
    package: 6,
  },
  {
    input: "TEEKANNE PREMIUM FRUIT SELECTION Tee 20 Portionen",
    package: 20,
  },
  {
    input:
      "LILLYDOO hautfreundliche Windeln Größe 5 (11-16 kg), Monatsbox (152 Windeln) (FSC-Mix)",
    package: 152,
  },
  {
    input: "STAR WARS - Mystery Bitty Pop (Box 36 pi?¿ces) (US IMPORT) ACC NEU",
    package: 36,
  },
  {
    input: "Red Bull Waldbeere Sugarfree Dose The Spring Edition 24x0,25l",
    package: 24,
  },
  {
    input: "MARIENDISTEL FRÜCHTE Filterbeutel 20X1.7 g",
    package: 20,
  },
  {
    input: "TEEKANNE frio Himbeere Zitrone Tee 18 Beutel à 2,5 g",
    package: 18,
  },
  {
    input: "1000er Multipack Briefumschlag DIN lang mF sk ws 1.000 St./Pack.",
    package: 1000,
  },
  {
    input: "HELLMA Sugar Balls Happy Zuckersticks 400 x 3,6 g",
    package: 400,
  },
  {
    input:
      "Lavazza Crema e Gusto, Arabica und Robusta, zart geröstet 5x10= 50 Kapseln",
    package: 50,
  },
  {
    input: "HP Nr. 302XL 3-farbig (F6U67AE)",
    package: null,
  },
  {
    input:
      "Arla Portionsmilch 100 x 2cl. Milch-Portion 1,5% Fett, 20ml im Displaykarton",
    package: 100,
  },
  {
    input: "Ferrero Kinder Riegel (36er-Packung)",
    package: 36,
  },
  {
    input: "NONGSHIM - Instant Nudeln Shin Ramyun - Multipack (20 X 120 GR)",
    package: 20,
  },
  {
    input:
      ":4x Campingaz CV 470 Plus Kartusche mit Ventil, 80:20 Butan:Propan, 450g (1,8kg)",
    package: 4,
  },
  {
    input:
      "Blink Outdoor – kabellose, witterungsbeständige HD-Überwachungskamera mit zwei Jahren Batterielaufzeit und Bewegungserfassung, funktioniert mit Alexa | 4 Kameras",
    package: 4,
  },
  {
    input: "Cottelli Collection Lingerie Hebe-Set ouvert aus Satin 2-teilig",
    package: null,
  },
  {
    input:
      "eufy Security S330 eufyCam 3, überwachungskamera aussen, 4K kabellose überwachungskamera solar 3+1 mit Gesichtserkennung K.I, Farbige Nachtsicht, Erweiterbarer Speicher 16TB, Gebührenfreie Nutzung",
    package: null,
  },
  {
    input: "Bosch GRA 18V2-46 2x18V (ohne Akku und Ladegerät)",
    package: null,
  },
  {
    input: "CARDO Freecom 4x Duo",
    package: null,
  },
  {
    input: "Niermann Wolke Bunte Blumen 5-flg./20LED",
    package: null,
  },
  {
    input:
      "Nexa Lotte Schrankfalle für Lebensmittelmotten, Mottenbekämpfung, Insektizidfrei, gegen Nahrungsmittelmotten, 2 Fallen",
    package: 2,
  },
  {
    input:
      "Palette Intensiv Creme Coloration 1-0/900 Schwarz Stufe 3 (3 x 115 ml), Permanente Haarfarbe für bis zu 8 Wochen intensiven Farbglanz & 100 % Grauabdeckung",
    package: 3,
  },
  {
    input:
      "Syoss Oleo Intense Öl-Coloration 4-86 Schokoladenbraun Stufe 3 (115 ml), dauerhafte Haarfarbe mit pflegendem Öl, Coloration ohne Ammoniak",
    package: null,
  },
  {
    input: "Scholl Einlegesohlen GelActiv Everyday, Gr. 40 -46,5 (1..., 2 St",
    package: 2,
  },
  {
    input: "essence Blush Baby Got Blush 10 Tickle Me Pink, 5,5 g",
    package: null,
  },
  {
    input: "ZEWA W&W Extra Lang Original 2x86",
    package: 2,
  },
  {
    input: "STAEDTLER Farbstifte Noris Club 127 12+4er-Bonuspack",
    package: null,
  },
  {
    input: "House of the Dragon - Staffel 1 [4 BRs]",
    package: null,
  },
  {
    input: "Illumination 5 Filme DVD-Edition [5 DVDs]",
    package: null,
  },
  {
    input: "Dragonball Z - TV-Serie - Vol.1 (Episoden 1-35) [4 BRs]",
    package: null,
  },
  {
    input: "BIALETTI Espressokocher Moka Express 6 Tassen",
    package: null,
  },
  {
    input: "Jada - Disney 100 Blind Pack Nanofigs, 13-fach sortiert, 1 Stück",
    package: 1,
  },
  {
    input:
      "Ceva Katzenzubehör Feliway Friends 3x30 Tage Vorteilspack Nachfüllflakon",
    package: 3,
  },
  {
    input: "LaSelva 6x Pesto mit Curry und Blüten - Kürbis Würzpaste 130g",
    package: 6,
  },
  {
    input:
      "COSRX Clear Fit Master Patch 18 Counts | Invisible Acne Pimple Pactches Pickel Pflaster Anti Akne Blemish A.D.F Hydrokolloid Hautpflaster Treatment Facial Spots Dots für Gesicht | Korean Skincare",
    package: 18,
  },
  {
    input:
      "10pcs Siddhalepa Ayurveda Wellness Kräuter Balsam VERSAND AUS DEUTSCHLAND",
    package: 10,
  },
  {
    input: "Anti Pickel Patches Master Clear Fit, 18 St",
    package: 18,
  },
  {
    input:
      "MjAMjAM - Premium Nassfutter für Katzen - Quetschie - Insekt mit saftigem Hühnchen, 10 x Stück, nachhaltig und getreidefrei, mit Insektenprotein",
    package: 10,
  },
  {
    input:
      "PURINA GOURMET Revelations Mousse, Katzenfutter nass in Sauce, mit Thunfisch, 12er Pack (12 x 2 Portionen à 57g)",
    package: 12,
  },
  {
    input:
      '5 er Pack-VARTA Batterie (1 x Lithium photo "Professional", CR-P2 6 V',
    package: 5,
  },
  {
    input:
      "Ansmann Alkaline Batterien AAA Industrial 1501-0004 (20 x Micro AAA LR03, 1,5 V, Kapazität: 1.200 mAh)",
    package: 20,
  },
  {
    input:
      "Neues AngebotSmartKeeper CL04P1GN Port Blocker Usb Type-C Green Plastic 10 Pc(S) ~E~",
    package: 10,
  },
  {
    input: "Smartkeeper CL04P1GN - Port Schloss, USB Typ C, 10 Stück, grün",
    package: 10,
  },
  {
    input:
      "10X SMK1 089 Fassung für LEDs 5mm Metall konvex IP67 SIGNAL-CONSTRUCT",
    package: 10,
  },
  {
    input:
      "8 Stk. Gnosjö Konstsmide WB LED-Lich­ter­vor­hang 3703-803 WB LED-Lich­ter­vor­hang",
    package: 8,
  },
  {
    input:
      "10X RTF-5010 Halter für LED 5mm Kunststoff konvex KINGBRIGHT ELECTRONIC",
    package: 10,
  },
  {
    input: "4x 3M Scotch Klebefilm Magic 811, ablösbar, 19 mm x 33 m",
    package: 4,
  },
  {
    input:
      "8er Multipack Maul Kleiderbügel 9453370 (B x H) 45 cm x 22 cm Holz Holz 8 St.",
    package: 8,
  },
  {
    input:
      "DOMETIC Power-Care für Camping-WCs & Chemietoiletten – Sanitär-Zusatz zersetzt Fäkalien und verhindert unangenehme Gerüche – Alternative zu Sanitärflüssigkeit (20 Tabs für 20 Tankfüllungen)",
    package: 20,
  },
  {
    input: "Dometic Powercare Tabs 20 Stck.",
    package: 20,
  },
  {
    input: "Vitakraft 25226 Kräcker Meerschweinchen Trio-Mix, 3Stk",
    package: 3,
  },
];
